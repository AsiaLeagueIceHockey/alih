...
### 5.10 온보딩 (Onboarding) 및 알림 개선 ⭐ NEW
- **진입 조건**: 로그인 했으나 `favorite_team_ids`가 없는 신규 유저
- **Step 1 (언어)**: 언어 선택
- **Step 2 (팀)**: 응원 팀 선택 (다중 선택 가능)
- **Step 3 (프로필)**: 닉네임 설정 (랜덤 추천 + 중복 체크)
- **알림 프롬프트**: 온보딩 완료 후 20초 뒤 하단 배너로 "홈 화면 추가" 유도
...
| 날짜 | 변경 내용 |
|------|----------|
| 2026-02-15 | UI/UX 개선: 온보딩 로직 수정(버그 픽스), 카카오 아이콘 통일, 회원탈퇴 숨김 🎨 |
| 2026-02-15 | 온보딩 3단계 개편 (닉네임 설정 추가) 👤 |
| 2026-02-15 | 승부 예측 기능 추가 (MatchPrediction, Lazy Registration) 및 UX 고도화 ⭐ |
